...
export default function Search({ data }) {
  ...
  const handleFilter = (e) => {
    ...
    const valName = name.current.value;
    const valSubject = subject.current.value;
    const valDate = date.current.value;

    const url = `?q=${valName}&subject=${valSubject}&date=${valDate}`;
    router.push(url);
  }
  ...
  return (
    <>
      <div className="w-full">
        ...
        <form onSubmit={handleFilter} className=" flex justify-between w-full mt-10">
          <div className="w-150">
            ...
            <div className="relative flex border h-12 shadow  pr-4 rounded-xl dark:text-gray-800 dark:border-gray-700 dark:bg-gray-200">
              ...
              >
                ...
                {data?.payload?.map((item, index) => (
                  <option
                    key={index}
                    className="font-medium h-12 shadow pl-12 pr-4 cursor-pointer"
                    value={item?.subjectId || "filter"}
                  >
                    {item?.subjectName}
                  </option>
                ))}
              ...
              <select onChange={handleFilter} ref={date} name="date" className="px-5">
